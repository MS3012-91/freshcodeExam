...
module.exports.validateRegistrationData = async (req, res, next) => {
  const validationResult = await schems.registrationSchema.isValid(req.body);
  if (!validationResult) {
...
module.exports.validateLogin = async (req, res, next) => {
  const validationResult = await schems.loginSchema.isValid(req.body);
  if (validationResult) {
    next();
  } else {
...
module.exports.validateContestCreation = (req, res, next) => {
  ...
  req.body.contests.forEach(el => {
    promiseArray.push(schems.contestSchema.isValid(el));
  });
